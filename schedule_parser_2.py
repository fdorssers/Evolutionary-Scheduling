...
def parse_constraint(lines, header_start, header_end, function, constraint_enum):
    constraints = {c: [] for c in constraint_enum}
    for line in lines[header_start + 1:header_end]:
        constraint = function([l.strip() for l in line.split(',')])
        constraints[constraint.constraint].append(constraint)
    return constraints
